    ...
    while not should_stop:
        ...
        if response.status_code == 200:
            ...
            for article_data in data['data']:
                article_datetime = datetime.datetime.strptime(article_data.get('date_unparsed', ''), "%Y-%m-%d %H:%M:%S")
                if last_scraped_datetime and article_datetime <= last_scraped_datetime:
                    should_stop = True
                    logger.info("Stopping the loop as article datetime is older than or equal to last scraped datetime.")
                    break
                ...
                article = N1Article(article_id=article_id, title=title, date=date, time=time, source=link, category=category)
...
def get_text_from_article(article_source):
    ...
    modified_text = text.replace('N1 pratite putem aplikacija za Android | iPhone/iPad i mreža Twitter | Facebook | Instagram | TikTok.', '')
    modified_text = modified_text.replace('Podijeli :', '')
    modified_text = modified_text.replace('\n', ' ')
    return modified_text

def get_tags_from_article(article_source):
    response = requests.get(article_source, headers={'Accept-Charset': 'UTF-8'})
...
article_list = collect_articles_from_api(base_url, params, last_scraped_datetime)
...
if article_list:
    ...
    save_last_scraped_datetime(datetime.datetime.strptime(last_article_datetime, "%Y-%m-%d %H:%M"))
